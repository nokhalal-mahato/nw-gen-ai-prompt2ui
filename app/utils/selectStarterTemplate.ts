...
export async function getTemplates(templateName: string, title?: string) {
  ...
  // exclude    .prompt2ui
  filteredFiles = filteredFiles.filter((x) => x.path.startsWith('.prompt2ui') == false);

  // check for ignore file in .prompt2ui folder
  const templateIgnoreFile = files.find((x) => x.path.startsWith('.prompt2ui') && x.name == 'ignore');
...
<prompt2uiArtifact id="imported-files" title="${title || 'Importing Starter Files'}" type="bundled">
${filesToImport.files
  .map(
    (file) =>
...
`;
  let userMessage = ``;
  const templatePromptFile = files.filter((x) => x.path.startsWith('.prompt2ui')).find((x) => x.name == 'prompt');
